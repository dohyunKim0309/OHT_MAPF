...
**예외 — 명시적 일괄 정리 위임.** 소유자가 "이 컨테이너의 `Data*`는 내가
소유했고 지금 다 지워도 된다"를 확인하고 부르는 *옵트인* 연산은 둘 수 있다
(예: [[avl_tree]]의 `clearAndDelete()`). 이때도 **소멸자는 여전히 비소유**이고,
delete를 *결정*하는 주체는 소유자다 — 컨테이너는 순회·delete *실행만 대행*한다.
"누가 지울지 결정"(소유자)과 "지우는 노동"(컨테이너 대행)을 분리한 것이라
비소유 원칙과 모순되지 않는다. 트리에서 포인터를 하나씩 빼 정리하는
번거로움을 피하기 위함이다.
