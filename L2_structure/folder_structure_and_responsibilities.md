...
- L4 모듈 문서는 L3와 basename이 겹친다(둘 다 `avl_tree.md`). L4로 링크할 땐 경로를
  명시(`[[L4_functions/avl_tree]]`)하고, 무자격 `[[avl_tree]]`는 L3 모듈 문서를 뜻한다.
...
- 임베드하는 문서와 **같은 폴더의 `assets/`**에 둔다.
  예: `L3_modules/data_structure/avl_tree.md` → `L3_modules/data_structure/assets/`.
...
### `L3_modules/`
- 모듈별 1문서 원칙
- 각 모듈: 책임, 선택 근거, 구현 방식, 외부 제공 기능, 의존성, 사용처
- 변경 빈도: 중간

### `L4_functions/`
- 함수 시그니처, 사전/사후 조건, 소유권, 필요시 의사코드
- **모듈별 1문서** (`L4_functions/<module>.md`). 문서를 **1) Fields, 2) Methods**로
  나누고 각각 **public/private** 하위에 항목별로 둔다. 각 함수 항목은 시그니처·사전/사후
  조건·소유권·의사코드. (확정 2026-06-06, 첫 사례 [[L4_functions/avl_tree]])
...
└── main.cpp
```
각 모듈의 파일 구성과 책임, 구체 Data 자식의 위치는 [[L3_modules]]의
해당 모듈 문서를 참조.
