...
{ // 1.1 TITLE
  ...
  // (subtitle: "Data Structure" — singular, per request)
...
{ // 3.1 WHAT DATA
  ...
  title(s, "A Samsung semiconductor-fab network in Yongin", { size: 30 });
...
{ // 5.5a AVL (figure)
  ...
  keyline(s, "Per-node occupancy as sorted, merged intervals.  Recorded by PP, read by BFS via isFree.", 2.0, { size: 15 });
...
{ // 5.5c COALESCING — Interval geometry (code)
  const s = newSlide(); sectionTag(s, "5 · Data structures  ·  AVL ★ merging (1/2)");
  title(s, "Merging — the geometry lives in Interval");
  keyline(s, "operator< (by start) · touches (ranges meet?) · merge (their union). Interval owns the geometry; the AVL never knows it.", 2.0, { size: 14 });
  imgBox(s, "interval_cpp.png", 1222, 1150, { x: 2.6, y: 2.65, w: 8.1, h: 4.5 });
}
{ // 5.5c-2 COALESCING — reserve() orchestrates (code)
  const s = newSlide(); sectionTag(s, "5 · Data structures  ·  AVL ★ merging (2/2)");
  title(s, "Merging — reserve() absorbs neighbors");
  // reserve()'s "absorb left" loop on the left
  imgBox(s, "reservationTable_merge_left.png", 1222, 840, { x: 0.7, y: 2.4, w: 6.4, h: 4.3 });
  s.addText("How reserve() folds", { x: 7.4, y: 2.5, w: 5.2, h: 0.4, margin: 0, fontFace: BODY, fontSize: 16, bold: true, color: INK });
  s.addText([
    { text: "predecessor / successor", options: { fontFace: BODY, fontSize: 15, color: ACC, bold: true, bullet: { code: "2022" }, breakLine: true } },
    { text: "find the touching neighbor in the AVL", options: { fontFace: BODY, fontSize: 13, color: MUTE, breakLine: true, paraSpaceAfter: 10 } },
    { text: "touches? → merge", options: { fontFace: BODY, fontSize: 15, color: ACC, bold: true, bullet: { code: "2022" }, breakLine: true } },
    { text: "absorb it, then remove the old node (we own it)", options: { fontFace: BODY, fontSize: 13, color: MUTE, breakLine: true, paraSpaceAfter: 10 } },
    { text: "repeat both sides → insert", options: { fontFace: BODY, fontSize: 15, color: ACC, bold: true, bullet: { code: "2022" }, breakLine: true } },
    { text: "one disjoint interval where several touched", options: { fontFace: BODY, fontSize: 13, color: MUTE } },
  ], { x: 7.4, y: 3.0, w: 5.2, h: 3.8, margin: 0, valign: "top" });
}
{ // 5.5d MEMORY — no leaks down the chain
  const s = newSlide(); sectionTag(s, "5 · Data structures  ·  AVL ★ memory");
  title(s, "No leaks: dynamic_array → AVL → Node → Data*");
  keyline(s, "Ownership is explicit; destruction walks the whole chain.  (ASan/UBSan: leak-free.)", 2.0, { size: 14 });
  // three captures, ordered as the destruction chain, with arrows
  imgBox(s, "reservationTable_memory_1.png", 1222, 578, { x: 0.7, y: 2.5, w: 5.9, h: 2.8 });
  s.addText("① table dtor → per-node clearAndDelete → delete[] trees", { x: 0.7, y: 5.35, w: 5.9, h: 0.4, margin: 0, fontFace: BODY, fontSize: 12, color: MUTE, italic: true });
  imgBox(s, "reservationTable_memory_2.png", 1204, 320, { x: 6.9, y: 2.5, w: 5.7, h: 1.5 });
  s.addText("② clearAndDelete → destroyAndDelete(root)", { x: 6.9, y: 4.05, w: 5.7, h: 0.4, margin: 0, fontFace: BODY, fontSize: 12, color: MUTE, italic: true });
  imgBox(s, "reservationTable_memory_3.png", 1010, 756, { x: 6.9, y: 4.5, w: 5.7, h: 2.6 });
  s.addText("③ post-order: delete each Data* (owned payload), then the node", { x: 6.9, y: 7.05, w: 5.7, h: 0.35, margin: 0, fontFace: BODY, fontSize: 12, color: MUTE, italic: true });
}
...
{ // 7.1 TAKEAWAY (dark)
  ...
  s.addText("Correctness rests on the reservation table's merge invariant; scale is bounded by data layout, not the search.", { x: 0.9, y: 4.7, w: 11.0, h: 0.8, margin: 0, fontFace: BODY, fontSize: 15, color: "CADCFC" });
}
