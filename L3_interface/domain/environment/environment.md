...
상세 정의는 [[agent]]. 요약: `id`/`priority`/`current`/**목표 시퀀스**를 묶은 `Data`
자식(`operator<`=priority). environment가 배열로 **소유**하며 목표 시퀀스를 무한
스트림으로 채운다. [[planner]]의 [[min_heap]]은 비소유로 `Agent*`를 빌린다.
...
1. PP에게 현재 상태로 전체 계획을 요청 → 각 에이전트의 H스텝 경로 수신. PP는 각
   에이전트의 **목표 시퀀스**를 `dwell` 정지와 함께 findPath에 넘긴다.
2. 각 에이전트를 그 경로대로 **C스텝 전진**시킨다.
3. C스텝 동안 **목표 도달**한 에이전트: 도달 카운트++, 해당 목표를 큐에서 소비하고
   (도달 시 `dwell`스텝 작업 정지는 경로에 이미 반영) **다음 랜덤 목표를 큐에 보충**.
   목표는 무한 스트림이라 에이전트는 절대 영구 정지하지 않는다(target conflict 회피).
...
- 새 목표 도달 가능성 — 원본 범위 `[0, originalSize())`에서 랜덤으로 뽑되
  (가상노드 제외), 강연결이 아니면 도달 불가 목표가 나올 수 있음. planner 실패 시 재추첨 등 처리 필요.
