...
> **구성 축**
> produced-by:: [[pathfinding|PathFinder]]
...
- 경로는 **목표 시퀀스**를 따라간다: 목표에 도달하면 `dwell`스텝 작업 정지(같은 노드
  반복) 후 다음 목표로 이동. 즉 같은 노드 반복 = 대기 또는 작업 정지.
- **무한 정지 없음**: 도달 후 유한 dwell만 정지하므로 경로가 한 노드에 영구 머물지
  않는다(target conflict 회피). 목표가 H 전에 소진될 때만 마지막 목표에서 정지.
- **빈 Path**(길이 0) = 시작조차 못 떠남. `path.empty()`.
...
- 구성: [[pathfinding|PathFinder]]에서 생산 → [[planner]] → [[environment]]로 관통.
