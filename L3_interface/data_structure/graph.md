...
**필드** (private):
- `int n` — 정점 수(원본 + 가상).
- `int originalCount` — 원본 노드 수. 인덱스 `< originalCount`이면 원본,
  그 이상이면 가상노드(전처리에서 원본 뒤에 붙임). 별도 expanded 플래그
  없이 인덱스 규약으로 원본/가상을 구분한다.
...
**methods**:
- 생성자 `Graph(int n, int originalCount)` — 정점 n개, 간선 없음.
- `void addEdge(int from, int to)` — 단위 간선 추가.
- `const 이웃목록& neighbors(int v) const` — 정점 v의 나가는 간선들. 순회용.
- `int size() const` — 정점 수(원본+가상).
- `int originalSize() const` — 원본 노드 수. 새 목표는 `[0, originalSize())`
  에서 뽑는다(가상노드는 목표가 될 수 없음 — 간선 중간 지점이므로).
- `bool isOriginal(int v) const` — `v < originalCount`.
