...
*공개(public):*
 ...
 - `bool contains(Data* key) const` — 존재 여부만. (`find`/`contains` 중 사용처가
   쓰는 쪽으로 정리. 잠정)
 - `Data* predecessor(Data* key) const` — `key`보다 작은 것 중 **최대** `Data*`
   (in-order 직전). 없으면 `nullptr`. `key`가 트리에 있든 없든 동작 — 병합이
   "삽입 전 좌측 이웃 확인"에 쓴다. O(log N).
 - `Data* successor(Data* key) const` — `key`보다 큰 것 중 **최소** `Data*`
   (in-order 직후). 없으면 `nullptr`. 우측 이웃 확인용. O(log N).
 - `void remove(Data* key)` — `operator<`로 같은 키 노드를 삭제 후 경로를 거슬러
   재균형. 자식이 둘이면 successor로 대체. 담은 `Data*`는 delete 안 함(비소유) —
   호출자가 받아 처리. reservation_table의 병합이 흡수된 이웃 제거에 쓴다. O(log N).
